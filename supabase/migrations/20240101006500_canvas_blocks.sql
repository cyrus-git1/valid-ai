-- 65_canvas_blocks.sql
-- Grounding canvas persistence: store the twelve business-canvas blocks as
-- kg_nodes, a near-clone of app_entities (migration 47). Same substrate, same
-- embedding space, same lookup-then-upsert idempotency. No new tables.
--
-- Storage model:
...
-- ── 3. upsert_canvas_block RPC (mirror upsert_app_entity, mig 47) ────────────
...
-- ── 4. canvas_by_scope RPC — fetch all blocks for a scope (not nearest) ──────
