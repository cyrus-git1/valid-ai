-- 50_fix_search_rpc_overloads.sql
-- FIX: "function public.hybrid_search_kg_nodes(...) is not unique".
--
-- Stale overloads of search_kg_nodes and hybrid_search_kg_nodes accumulated
-- across migrations (a `drop function` that didn't match an older signature left
-- the old version behind while a new one was created). With >1 overload,
-- PostgREST cannot choose a candidate, so EVERY rpc() call to these functions
-- failed at runtime — the retriever caught the error and returned [], making
-- /search/graph silently return zero results for all queries.
--
-- Robust fix: drop ALL overloads of both functions by name (regprocedure loop),
-- then recreate exactly one of each. The recreated definitions below are the
-- migration-49 versions (embedding + content in the output); their own
-- `drop function if exists (<specific sig>)` lines are harmless no-ops now.
...
begin
  for r in
    ...
    where n.nspname = 'public'
      and p.proname in ('search_kg_nodes', 'hybrid_search_kg_nodes')
  loop
    execute 'drop function ' || r.sig::text || ' cascade';
  end loop;
end $$;

-- ── Recreate the single correct definitions (identical to migration 49) ──────
